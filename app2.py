...
if check_password():
    ...
    if st.session_state['usercheck']==True:
        if st.session_state["st"]==True:
            ...
            Qa=st.radio(questions[str(st.session_state["rn"][0])],random.sample([x[0] for x in choices[str(st.session_state["rn"][0])]],k=len([x[0] for x in choices[str(st.session_state["rn"][0])]]))
